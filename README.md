...
- 游戏内悬浮面板，实时显示当前金币
- 输入目标金币数量，一键写入
- 自动检测并挂钩 D3D11 / D3D9（本游戏使用 D3D11）
- 单文件交付（DLL 内嵌进 exe），目标环境仅需 **Windows 10**
- 支持 32 位游戏进程
...
> `*.exe` / `*.dll` / `*.o` 为构建产物，已加入 `.gitignore`，不入库；运行 `build_launcher.bat` 即可生成。

## 快速使用

1. 启动游戏（Beat Cop）
2. 以管理员身份运行 `Beat Cop.exe`（构建产物，见下）
...
环境要求：Windows 10 + [MSYS2](https://www.msys2.org)（一次性安装），并安装 32 位工具链：

```bash
pacman -S mingw-w64-i686-gcc
```

双击脚本即可（脚本会自动查找 MSYS2，项目文件夹可放在任意位置）：
